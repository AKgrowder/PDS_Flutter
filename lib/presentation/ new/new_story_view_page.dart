          ...
          children: [
            Expanded(
              ...
              storyItems: List.generate(widget.data.images.length, (index) {
                print(
                    "story view count : ${widget.data.images[index].storyViewCount!}");

                return widget.data.images[index].image!.endsWith(".mp4")
                    ? StoryItem.pageVideo(widget.data.images[index].image!,
                        controller: controller,
                        duration: Duration(
                            seconds: widget.data.images[index].duration!),
                        views:
                            widget.data.images[index].userUid == widget.userId
                                ? widget.data.images[index].storyViewCount ?? 0
                                : -1,
                        index: index,
                        caption: Material(
                    ...
                    : StoryItem.pageImage(
                        ...
                        views:
                            widget.data.images[index].userUid == widget.userId
                                ? widget.data.images[index].storyViewCount!
                                : -1,
                        index: index,
                        onTap: () async {
                          controller.pause();
              ...
              onComplete: () {
                ...
                if (widget.index + 1 < widget.datas.length &&
                    widget.userId !=
                        widget.datas[widget.index].images[0].userUid) {
                  Navigator.pushReplacement(context,
                      MaterialPageRoute(builder: (context) {
                    return NewStoryViewPage(widget.datas[widget.index + 1],
                        widget.datas, widget.index + 1, widget.userId);
                  }));
                } else {
        ...
        showDragHandle: false, */
            ...
            builder: (context) {
              return StatefulBuilder(
                ...
                return BlocConsumer<ViewStoryCubit, ViewStoryState>(
